...
def get_std_dev(ticker_symbol, period, filename):
    if os.path.isfile(filename):
        stock_data = pd.read_csv(filename, header=0, index_col=0)
    else:
...
async def on_message(message):
    ...
    if message.content.startswith('!sd'):
        ...
        for period in valid_periods:
            current_price = get_current_price(ticker_symbol, period=period)
            ...
            std_devs[period] = {'price_sd': f"{price_std_dev:.2f}", 'upper_price': upper_price, 'lower_price': lower_price, 'percentage_sd': f"{percentage_std_dev:.2f}", 'company_name': company_name}

        company_name = std_devs['1wk']['company_name']
        message_chunks = [
            f"Volatility for **{company_name}**\n",
